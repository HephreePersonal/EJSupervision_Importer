-- Drop table If exists
	DROP TABLE IF EXISTS ELPaso_TX.dbo.PrimaryKeyScripts_Operations;
-- Create Table to store scripts
	CREATE TABLE ELPaso_TX.dbo.PrimaryKeyScripts_Operations
		(
 ...
 -- Generate PK scripts using FOR XML PATH 
	INSERT INTO ELPaso_TX.dbo.PrimaryKeyScripts_Operations (ScriptType,DatabaseName,SchemaName,TableName,Script)
			SELECT 'PK','ELPaso_TX',S.[NAME],T.[NAME],
				'ALTER TABLE [ELPaso_TX].[dbo].[Operations_' + t.[NAME] + '] ADD CONSTRAINT [Op_' + kc.[NAME] + '] PRIMARY KEY (' +STUFF((SELECT ', [' + c2.[NAME] + ']' FROM Operations.sys.index_columns ic2 JOIN Operations.sys.columns c2 ON c2.object_id=ic2.object_id AND c2.column_id=ic2.column_id WHERE ic2.object_id=t.object_id AND ic2.index_id=kc.unique_index_id ORDER BY ic2.key_ordinal FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') + ')' AS Script 
			FROM 
