...
def create_primary_keys(conn, config, log_file):
    ...
    for idx, row in enumerate(tqdm(rows, desc="Executing Target PK Creation", unit="table"), 1):
        ...
        try:
            ...
            error_msg = f"Error executing PK statements for row {idx} (Operations.{full_table_name}): {e}"
            logger.error(error_msg)
            log_exception_to_file(error_msg, log_file)
...
def show_completion_message():
    ...
    proceed = messagebox.askyesno(
        ...
        "Operations database migration is complete.\n\n"
        "You may now drop the Operations database if desired.\n\n"
    ...
    try:
        ...
        try:
            with get_target_connection() as target_conn:
                # Step 1: Gather document IDs
                gather_document_ids(target_conn, config)
